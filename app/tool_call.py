"""
工具调用模块 — WorkBuddy Desktop-api

将 OpenAI function calling 格式转译为 WorkBuddy 可理解的 WorkBuddyML 提示词，
并从 WorkBuddy 的纯文本响应中解析回结构化 tool_call。
...
def build_tool_prompt(tools: List[Dict[str, Any]], passthrough: bool = False) -> str:
    ...
    passthrough=True：完全依赖上游原生 OpenAI tools 协议。
      ...
      WorkBuddyML 说明书或英文指令，避免与上游原生 tool_calls 响应冲突。
      适用于上游已实现 OpenAI 原生 tool_calls 协议的 WorkBuddy 上游
      （`copilot.tencent.com/v2/chat/completions` 实测支持）。

    passthrough=False：在 prompt 中嵌入 WorkBuddyML 文本格式说明书，
      让模型在正文里输出 `<|WorkBuddyML|tool_calls>...</|>` 供
...
<|WorkBuddyML|tool_calls>
  <|WorkBuddyML|invoke name="TOOL_NAME_HERE">
    <|WorkBuddyML|parameter name="PARAMETER_NAME"><![CDATA[PARAMETER_VALUE]]></|WorkBuddyML|parameter>
  </|WorkBuddyML|invoke>
</|WorkBuddyML|tool_calls>

RULES:
1) Use the <|WorkBuddyML|tool_calls> wrapper format.
2) Put one or more <|WorkBuddyML|invoke> entries under a single <|WorkBuddyML|tool_calls> root.
3) Put the tool name in the invoke name attribute: <|WorkBuddyML|invoke name="TOOL_NAME">.
4) All string values must use <![CDATA[...]]>, even short ones.
5) Every top-level argument must be a <|WorkBuddyML|parameter name="ARG_NAME">...</|WorkBuddyML|parameter> node.
...
10) If you call a tool, the first non-whitespace characters must be exactly <|WorkBuddyML|tool_calls>.
11) Never omit the opening <|WorkBuddyML|tool_calls> tag.

【WRONG — Do NOT do these】:

Wrong 1 — mixed text after XML:
  <|WorkBuddyML|tool_calls>...</|WorkBuddyML|tool_calls> I hope this helps.

Wrong 2 — Markdown code fences:
  ```xml
  <|WorkBuddyML|tool_calls>...</|WorkBuddyML|tool_calls>
  ```

Wrong 3 — missing opening wrapper:
  <|WorkBuddyML|invoke name="TOOL_NAME">...</|WorkBuddyML|invoke>
  </|WorkBuddyML|tool_calls>

Remember: The ONLY valid way to use tools is the <|WorkBuddyML|tool_calls>...</|WorkBuddyML|tool_calls> block.
...
def extract_tool_call(
    ...
    """从 WorkBuddy 输出文本中提取工具调用。

    5 重策略（按优先级）：
      0. <|WorkBuddyML|tool_calls> XML — WorkBuddyML 格式（最高优先级）
      1. TOOL_CALL: name(args)  — 旧格式兜底
      2. JSON {"name":"x","arguments":{...}} — 内嵌 JSON
      3. <tool_call> XML — WorkBuddy 原生格式
    ...
    # 策略0: WorkBuddyML 格式（最高优先级）
    tc = _extract_workbuddyml_tool_call(text, tool_names)
    if tc:
    ...
    # 无工具调用：若含 WorkBuddyML / 工具标记残留，仍清理再返回
...
def _has_tool_markers(text: str) -> bool:
    """检测文本是否包含工具调用标记（WorkBuddyML、XML 等）。"""
    markers = [
        "TOOL_CALL:", "<tool_call", "<function_call", "<function=",
        "<|WorkBuddyML|", "<tool_calls>", "<invoke",
    ]
...
def _extract_workbuddyml_tool_call(
    text: str, tool_names: List[str]
) -> Optional[List[Dict[str, Any]]]:
    """匹配 WorkBuddyML 格式:
    <|WorkBuddyML|tool_calls>
      <|WorkBuddyML|invoke name="TOOL">
        <|WorkBuddyML|parameter name="X"><![CDATA[V]]></|WorkBuddyML|parameter>
      </|WorkBuddyML|invoke>
    </|WorkBuddyML|tool_calls>
    """
    if not (_WORKBUDDYML_KEYWORD in text.lower() or
    ...
    # 缺失开标签修复：有关闭标签但缺开头 → 补回 <|WorkBuddyML|tool_calls>
    has_closing = ("</|WorkBuddyML|tool_calls>" in text or
                   "</workbuddyml-tool_calls>" in text.lower() or
                   "</tool_calls>" in text.lower())
    has_opening = ("<|WorkBuddyML|tool_calls" in text.replace(' ', '').replace('｜','|').replace('||','|') or
                   "<|WorkBuddyML|function_calls" in text.replace(' ', '').replace('｜','|').replace('||','|') or
                   "<workbuddyml-tool_calls" in text.lower() or
                   "<workbuddyml-function_calls" in text.lower() or
                   "<tool_calls>" in text.lower() or
                   "<function_calls>" in text.lower())
    if has_closing and not has_opening:
        text = "<|WorkBuddyML|tool_calls>\n" + text

    normalized = strip_workbuddyml(text)
    tool_calls = []
    ...
    if not blocks:
        ...
        for m in list(invoke_bare.finditer(normalized)) + list(invoke_bare_nospace.finditer(normalized)):
            ...
            if _is_inside_think(normalized, m.start()):
                continue
            inner = m.group(2)
            args = _parse_workbuddyml_parameters(inner)
    ...
    for block_text, _, _ in blocks:
        ...
        # Fallback: invokes without closing </invoke> (model bug — WorkBuddy sometimes omits closing tags)
        if not tool_calls:
...
# ─── 策略3 + 策略3.5: <tool_call> XML（WorkBuddy 原生 + Roo Code 格式）───
...
def _extract_xml_tool_call(
    ...
    WorkBuddy 原生: <tool_call><function=NAME><parameter=K>V</parameter>...</function></tool_call>
    ...
    # 尝试 WorkBuddy 原生格式: <function=NAME>
...
def _fixup_skill_params(func: Dict[str, Any]) -> None:
    ...
    WorkBuddy 模型偶尔把 skill_view 的 name 参数写成 skill_name，
    这里自动重映射，兼容两种写法。
    """
...
def clean_tool_text(text: str) -> str:
    ...
    # WorkBuddyML 标签残留（修复 \\| → \| 使正则正确匹配 WorkBuddyML 标签）
    text = re.sub(r"</?\|WorkBuddyML\|[^>]*>", "", text)
    text = re.sub(r"<tool_calls?>.*?</tool_calls?>", "", text, flags=re.DOTALL | re.IGNORECASE)
    # 同时匹配 <invoke> 和 <|WorkBuddyML|invoke> 两种形式
    text = re.sub(r"<(?:[|]WorkBuddyML[|])?invoke[^>]*>.*?</(?:[|]WorkBuddyML[|])?invoke>", "", text, flags=re.DOTALL | re.IGNORECASE)
    # 同时匹配 <parameter> 和 <|WorkBuddyML|parameter> 两种形式
    text = re.sub(r"<(?:[|]WorkBuddyML[|])?parameter[^>]*>.*?</(?:[|]WorkBuddyML[|])?parameter>", "", text, flags=re.DOTALL | re.IGNORECASE)
...
# ═══════════════════════════════════════════════════════════
# WorkBuddyML 噪声容错前缀：重复 <、|、｜、空格、workbuddyml 关键字
_WORKBUDDYML_NOISE_CHARS = set("|｜ \t\r\n")
_WORKBUDDYML_KEYWORD = "workbuddyml"
_WORKBUDDYML_KEYWORD_LEN = len(_WORKBUDDYML_KEYWORD)
# DSML 变体（模型有时用 DSML 替代 WorkBuddyML）
_DSML_KEYWORD = "dsml"
...
def _has_ml_keyword(text: str) -> bool:
    t = text.lower()
    return _WORKBUDDYML_KEYWORD in t or _DSML_KEYWORD in t or "function_calls" in t

# 连字符变体：workbuddyml-tool_calls → tool_calls
_WORKBUDDYML_HYPHENATED = {
    "workbuddyml-tool_calls": "tool_calls",
    "workbuddyml-tool-calls": "tool_calls",
    "workbuddyml-invoke": "invoke",
    "workbuddyml-parameter": "parameter",
}

_WORKBUDDYML_TAG_NAMES = {"tool_calls", "function_calls", "invoke", "parameter", "invokename"}
_CDATA_OPEN = "<![CDATA["
_CDATA_CLOSE = "]]>"


def strip_workbuddyml(text: str) -> str:
    """去除 WorkBuddyML 前缀标记，转换为标准 XML，支持噪声容错。

    处理的变体：
      <|WorkBuddyML|tool_calls>      →  <tool_calls>
      <WorkBuddyML|tool_calls>       →  <tool_calls>     (缺开头 |)
      <<|WorkBuddyML|tool_calls>     →  <tool_calls>     (重复 <)
      <|WorkBuddyML tool_calls>      →  <tool_calls>     (空格)
      <｜WorkBuddyML｜tool_calls>     →  <tool_calls>     (全宽管道)
      <WorkBuddyMLtool_calls>        →  <tool_calls>     (无分隔符)
      <|WorkBuddyML|tool_calls|>     →  <tool_calls>     (尾部 |)
      <workbuddyml-tool_calls>       →  <tool_calls>     (连字符)

    自动跳过 markdown 围栏代码块内的 WorkBuddyML 示例。
    """
    if not text:
        return text
    ...
    while i < n:
        ...
        j, is_workbuddyml = _consume_workbuddyml_noise(rest, text_lower, i + 1 + (1 if closing else 0))

        if is_workbuddyml:
            tag_name = _match_workbuddyml_tag(rest, j)
            if tag_name:
...
def _consume_workbuddyml_noise(rest: str, text_lower: str, abs_pos: int) -> tuple:
    """消费 WorkBuddyML 噪声前缀，返回 (j, is_workbuddyml)。"""
    j = 0
    is_workbuddyml = False
    rest_len = len(rest)
    ...
    while j < rest_len:
        ...
        if ch == '<':
            ...
            continue
        # 单字符噪声（|、空格等）
        if ch in _WORKBUDDYML_NOISE_CHARS:
            j += 1
            is_workbuddyml = True
            continue
        # 全宽管道 ｜(U+FF5C)
        if rest[j:].startswith('｜'):
            ...
            is_workbuddyml = True
            continue
        # workbuddyml / dsml 关键字
        if (rest[j:j+_WORKBUDDYML_KEYWORD_LEN].lower() == _WORKBUDDYML_KEYWORD or
                rest[j:j+_DSML_KEYWORD_LEN].lower() == _DSML_KEYWORD):
            kw_len = _DSML_KEYWORD_LEN if rest[j:j+_DSML_KEYWORD_LEN].lower() == _DSML_KEYWORD else _WORKBUDDYML_KEYWORD_LEN
            j += kw_len
            is_workbuddyml = True
    ...
    return j, is_workbuddyml


def _match_workbuddyml_tag(rest: str, j: int) -> Optional[tuple]:
    """匹配 WorkBuddyML 标签名，返回 (canonical_name, end_pos) 或 None。"""
    rest_len = len(rest)
    # 先检查连字符变体
    for hyphenated, canonical in _WORKBUDDYML_HYPHENATED.items():
    ...
    if tag_name in _WORKBUDDYML_TAG_NAMES:
        return (tag_name, name_end)

    return None


def _parse_workbuddyml_parameters(inner_text: str) -> Dict[str, Any]:
