"""会话管理 — 消息指纹续接 WorkBuddy conversationId

通过 SHA256 指纹检测消息连续性，复用 WorkBuddy 后端会话，
避免每次都生成新 conversationId 导致上下文丢失。

参考实现：
  GoblinHonest/workbuddy2api_workbuddyapi — session.ts / session-marker.ts
  (https://github.com/GoblinHonest/workbuddy2api_workbuddyapi)
"""
...
# token 超限后强制清屏（WorkBuddy 上下文 ~128K，留余量）
...
def get_or_create_session(
    ...
    Returns:
        ...
        - conversation_id: WorkBuddy 会话 ID（复用或新建）
