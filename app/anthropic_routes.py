"""
Anthropic Messages API 路由 — WorkBuddy Desktop-api

将 Anthropic Messages API 格式请求转换为 WorkBuddy API 调用并转换回 Anthropic 格式。

转换路径:
  Anthropic Request → convert_request() → OpenAI dict
    → build_query_from_messages() + WorkBuddyClient
  WorkBuddy Response → OCR/Analyze → OpenAI dict → convert_response() → Anthropic Response
"""
import json
import uuid
import time
...
from .routes import (
    ...
    _strip_tool_name_prefix, _strip_workbuddy_prefix, _safe_flush,
    validate_api_key,
)

router = APIRouter()

# ── Anthropic 模型名 → WorkBuddy Desktop 模型名映射 ──
...
ANTHROPIC_MODEL_ALIASES = {
    ...
    "claude-opus-4-7-search": "hy4-preview",
    "claude-opus-4-6-search": "hy4-preview",
    "claude-sonnet-4-7-search": "hy3",
    "claude-sonnet-4-6-search": "hy3",
    "claude-sonnet-4-7-nothinking": "hy3",
    "claude-sonnet-4-6-nothinking": "hy3",
...
def _resolve_anthropic_model(model: str) -> str:
    ...
    if m.startswith("claude-"):
        return "hy4-preview" if "opus" in m else "hy3"
...
# ─── 流式转换：WorkBuddy SSE → Anthropic SSE ───────────────────────
...
async def _anthropic_stream_think_wrapper(
    workbuddy_stream: AsyncIterator[dict],
    model: str,
    msg_id: str,
    tool_names: list = None,
) -> AsyncIterator[str]:
    """
    将 WorkBuddy 的流式事件（含 <think> 标签 + 可能含 TOOL_CALL）实时转换为 Anthropic SSE 事件。
        ...
        if has_tools and sieve:
            for ev in sieve.feed(buf_text):
                if ev.type == 'text':
                    ...
                    clean = _strip_workbuddy_prefix(clean)
    ...
    async for ev in workbuddy_stream:
        ev_type = ev.get("type")
...
async def anthropic_messages(
    ...
    model = body.get("model", "hy4-preview")
    ...
    if not account:
        raise HTTPException(
            status_code=503,
            detail=_anthropic_error_response("no workbuddy account", "service_error"),
        )
    ...
    try:
        ...
        content = _strip_workbuddy_prefix(content)
        ...
        raise HTTPException(
            ...
            detail=_anthropic_error_response(f"WorkBuddy API: {e.response_body[:200]}", "api_error"),
        )
    except Exception as e:
        import traceback
        traceback.print_exc()
...
async def anthropic_create_batch_ep(request: Request):
    ...
    async def _process_one(req):
        ...
        account = config_manager.get_next_account()
        if not account:
            return _anthropic_error_response("no workbuddy account", "service_error")
