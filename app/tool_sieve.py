...
模式：
  ...
  - 'workbuddyml':   检测 <|WorkBuddyML|tool_calls> / <tool_calls>
...
class StreamSieve:
    ...
    def _is_capture_complete(self) -> bool:
        ...
        if buf.lstrip().startswith("<"):
            # WorkBuddy 原生格式: <function=NAME>...</function>
            ...
            if "<tool_calls>" in buf and "</tool_calls>" in buf:
                return True
            if "<|WorkBuddyML|tool_calls>" in buf and "</|WorkBuddyML|tool_calls>" in buf:
                return True
            return False
    ...
    def _extract_non_tool_parts(self, text: str) -> Tuple[str, str]:
        ...
        elif "<|WorkBuddyML|tool_calls>" in rest:
            close_pos = rest.find("</|WorkBuddyML|tool_calls>")
            if close_pos >= 0:
                end = start + close_pos + len("</|WorkBuddyML|tool_calls>")
